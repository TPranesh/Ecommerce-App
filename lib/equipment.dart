...
class EquipmentPage extends StatelessWidget {
  ...
  Future<List<dynamic>> _fetchEquipmentProducts() async {
    try {
      final response = await http.get(
        Uri.parse('http://127.0.0.1:8000/api/products?category=equipment'),
        headers: {'Accept': 'application/json'},
      ).timeout(const Duration(seconds: 10));

      print('Equipment API Response: ${response.statusCode} - ${response.body}');
          ...
          children: [
            Container(
              ...
              decoration: const BoxDecoration(
                image: DecorationImage(
                  image: AssetImage('assets/images/messi.jpeg'), // Placeholder; replace with equipment_banner.jpg
                  fit: BoxFit.cover,
                ),
              ),
              child: Align(
            ...
            FutureBuilder<List<dynamic>>(
              future: _fetchEquipmentProducts(),
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  ...
                  return const Center(child: Text('No equipment found', style: const TextStyle(color: Colors.white)));
                } else {
